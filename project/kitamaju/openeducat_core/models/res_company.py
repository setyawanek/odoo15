...
class ResUsers(models.Model):
    ...
    def _department_count(self):
        return self.env['km.department'].sudo().search_count([])

    student_line = fields.Many2one('km.student', 'Line')
    user_line = fields.One2many('km.student', 'user_id', 'User Line')
    child_ids = fields.Many2many(
        'res.users', 'res_user_first_rel1',
        'user_id', 'res_user_second_rel1', string='Childs')
    dept_id = fields.Many2one('km.department', string='Department Name')
    department_ids = fields.Many2many('km.department',
