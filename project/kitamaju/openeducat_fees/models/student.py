...
class OpStudentFeesDetails(models.Model):
    _name = "km.student.fees.details"
    _description = "Student Fees Details"
    _rec_name = 'student_id'

    fees_line_id = fields.Many2one('km.fees.terms.line', 'Fees Line')
    ...
    course_id = fields.Many2one('km.course', 'Course', required=False)
    batch_id = fields.Many2one('km.batch', 'Batch', required=False)
    ...
    def get_invoice(self):
        ...
        element_id = self.env['km.fees.element'].search([
...
class OpStudent(models.Model):
    ...
    fees_detail_ids = fields.One2many('km.student.fees.details',
                                      'student_id',
                                      string='Fees Collection Details')
    fees_details_count = fields.Integer(compute='_compute_fees_details')

    @api.depends('fees_detail_ids')
    def _compute_fees_detail_ids(self):
        for rec in self.fees_detail_ids:
            print(rec.invoice_id)

    # @api.depends('fees_detail_ids')
    # def _compute_fees_details(self):
    #     for fees in self:
    #         fees.fees_details_count = self.env['km.student.fees.details'].search_count(
    #             [('student_id', '=', self.id)])
    ...
    #         'name': 'Fees Details',
    #         'view_mode': 'tree',
    #         'res_model': 'km.student.fees.details',
